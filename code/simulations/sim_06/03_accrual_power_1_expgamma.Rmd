...
Simulate the power of a trial with a time to event response where the trial has a superiority check at the interims. If the probability of expected success is above a specified threshold value then the trial is stopped. 

Assume some nominal sample size at each interim, say 100, 150, 200, 250, 300 and run a time to event analysis with subjects all followed up for a maximum of z months.
...
# Pregenerate data
d <- lapply(1:nsim, trial_dat, N, t0, delta, enro_rate = 10)
summary(d[[1]])
...
```{r}
d <- lapply(1:nsim, trial_dat, N, t0, delta, enro_rate = 14)
summary(d[[1]])
cl <- makeCluster(parallel::detectCores() , outfile="")
registerDoParallel(cl)
#registerDoSEQ()
...
```{r}
d <- lapply(1:nsim, trial_dat, N, t0, delta, enro_rate = 17)
summary(d[[1]])
cl <- makeCluster(parallel::detectCores() , outfile="")
registerDoParallel(cl)
#registerDoSEQ()
...
Shows the relationship between power and accrual in tte context. With moderate time to events (i.e. not instantaneous) slower accrual (generally) implies increase in power. Also, mean sample size (at which a decision for es can be made) increases.
