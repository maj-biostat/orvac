...
For the immunological endpoint we assess whether the difference in seroconversion rates is greater than 0 (treatment versus control, i.e. one-sided). Stopping venous sampling was based on a threshold of 0.95, as was the final analysis. For the futility check, each trial simulated conditional on the current posterior was assessed against a threshold of 0.95 and if less than 5% of the trials were successful then futility was declared.
...
Contrastingly, Table 5 contains the results when the futility rule is remove on both endpoints. The results suggest that without the futility rule, the type-i error would not be controlled at the 0.05 level. Also, the trials run for much, much longer.
...
Note that the coloured points indicate the results from a new simulations that I initiated after identifying a minor, but not ignorable, bug in the code. The next run of simulation results will only show results from the new codebase. 
...
Table 6 shows the combined set of results generated in these simulations post 2019-03-13. These simulations were generated to see the effect of a minor bug fix in the code and constitute the latest most relevant results. The next run of simulation results will only show results from the new codebase. 
