...
for(fname in resfiles){
  ...
  warningsPresent <- ifelse(warningsPresent == FALSE && length(d1$warnings) > 0, TRUE, FALSE)
  ...
  # sample sizes associated with each outcome
  df_outcome$ss_clin <- ifelse(df_outcome$inconclu == 1 & df_outcome$ss_clin == 0, df_outcome$look, df_outcome$ss_clin)
...
kable(df_0 %>% dplyr::select(-fname, -idsim, -nsim, -alt_cen) %>%
...
```{r, echo = F, eval = T}
df_tmp <- df_fig1 %>% 
  dplyr::filter(simname == "Null01")
ggplot(df_tmp, aes(x = ss_clin))+
  geom_histogram()

ggplot(df_tmp, aes(x = ss_immu))+
  geom_histogram()
