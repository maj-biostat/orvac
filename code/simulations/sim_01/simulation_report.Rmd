...
Power associated with log-rank test shown at sample sizes matching expected SS from Bayesian trial having accrual of 40 per quarter.
...
df_tmp <- df_all %>%
  dplyr::mutate(delta = trtclin - baseclin,
    gamma = trtsero - basesero) %>%
  dplyr::filter(delta > 0 & !(accrual %in% c(30, 50)) & info_delay != 0.8 & delta == 5 & basesero < 0.8)
...
df_tmp2 <- df_all %>%
  dplyr::mutate(delta = trtclin - baseclin,
    gamma = trtsero - basesero) %>%
  dplyr::filter(delta > 0 & !(accrual %in% c(30, 50)) & 
                  info_delay == 0 & delta == 5 & basesero < 0.8)
...
ggplot(df_tmp, aes(x = gamma, y = p_i_intrm_vs_fin, 
                   group = paste0(accrual), linetype = paste0(accrual)))  +
  geom_line() + 
  scale_linetype_discrete(name = "Accrual (n per qtr)") +
  scale_x_continuous("Difference seroconversion rate") +
  scale_y_continuous("Probability final analysis aligns with interim decision", 
