...
```{r, echo = F}
dpwr1 <- data.frame(p0 = c(0.3,0.3,0.3),
                    p1 = c(0.35,0.4,0.5),
                    diff = c(0.05,0.1,0.2),
                    ss = c(126,162,154),
                    pwr = c(0.15,0.38, 0.82))
dpwr1 <- dpwr1[order(dpwr1$diff), ]
digits <- c(2, 2, 2, 0, 2)
options(knitr.kable.NA = '-')
```
...
# Alternative null set - no futility rule
no_fut_set <- as.POSIXct(c("2019-03-16 01:53:36",
                "2019-03-16 17:01:30",
                "2019-03-16 22:18:47",
                "2019-03-17 03:29:54",
                "2019-03-17 08:44:10",
                ...
                "2019-03-18 00:27:09"))
df_1 <- df_0 %>%
  dplyr::filter(date %in% no_fut_set)

df_1b <- df_all %>%
  dplyr::filter(date %in% no_fut_set)

stopifnot(nrow(df_1) == 8)

# Alternative full set with post threshold fix
bug_fixed_set <- as.POSIXct(c("2019-03-16 11:54:14",
"2019-03-16 12:12:21",
"2019-03-16 12:29:46",
"2019-03-16 12:50:04",
"2019-03-16 13:05:13",
...
df_2 <- df_all %>%
  dplyr::filter(date %in% bug_fixed_set)

df_2a <- df_0 %>%
  dplyr::filter(date %in% bug_fixed_set)

stopifnot(nrow(df_2) == 44)
...
# post_final_thresh           used in the final analysis to assess both sero and tte endpoint posteriors
# 
# post_sero_win_thresh_start  we linearly interpolate between post_sero_win_thresh_start and 
#                             post_sero_win_thresh_end to come up with a post_sero_win_thresh threshold 
#                             at each interim. currently both set to 0.95 
# post_sero_win_thresh_end    
# post_sero_win_thresh        currently set to 0.95 for all interims up to nmaxsero then NA. used to check 
#                             whether a trial has 'won' during the futility assessment
# 
# post_tte_win_thresh_start   we linearly interpolate between post_tte_win_thresh_start and post_tte_win_thresh_end 
#                             to come up with a post_tte_win_thresh threshold at each interim. currently both set to 0.95 
# post_tte_win_thresh_end     
# post_tte_win_thresh         currently set to 0.95 for all interims. used to check whether a trial has 'won' during the futility assessment
# 
# pp_sero_sup_thresh          currently set to 0.95 for all interims. used to assess whether seroconversion ep should be stopped for superiority
# 
# post_tte_sup_thresh_start   we linearly interpolate between post_tte_sup_thresh_start and post_tte_sup_thresh_end 
#                             to come up with a post_tte_sup_thresh threshold at each interim. currently both set to 0.96
# post_tte_sup_thresh_end
# post_tte_sup_thresh         currently set to 0.96 for all interims. used to assess whether seroconversion ep 
#                             should be stopped for expected superiority
# 
# pp_sero_fut_thresh          futility check will stop the trial if the proportion of simulated trials from the 
#                             posterior predictive that 'win' is less than this threshold (<5% => futility)
# pp_tte_fut_thresh
...
d_thres <- data.frame(threshname = c("post_final_thresh",
                                     ...
                                     "pp_sero_fut_thresh",
                                     "pp_tte_fut_thresh"), 
                      thresh = c(0.95, 0.95, 0.96, 0.95, 0.95, 0.05, 0.05),
...
Null scenarios showing control of type-i error < 0.05 on the clinical endpoint. However, note that the type-i for stopping venous sampling is > 0.05 for some scenarios. Therefore, I am going to increase the threshold used to assess whether we should stop venous sampling due to superiority from 0.95 to 0.96. 

```{r, echo = F, eval = T}

kable(df_0 %>% 
        dplyr::filter(idsim %in% c("N01", "N02", "N03", "N04", "N05", "N06")) %>%
        dplyr::filter(date <= max_file_date) %>%
...
Contrastingly, Table 5 contains the results when the futility rule is remove on both endpoints. The results suggest that without the futility rule, the type-i error would not be controlled at the 0.05 level.
...
kable(df_1 %>% 
        dplyr::select(-idsim, -nsim, -alt_cen, -date) %>%
    dplyr::arrange(desc(fname)),
      caption = "Table 5. Simulation Null Scenarios without futility check",
      col.names = c("ctl", "trt", "ctl", "trt", "accrual", "delay",
                    "e.s.", "e.f", "s.v.", "n.d", 
                    "enrolled", "blood res", "P(<250)", "P(<500)", "P(<750)", "evnts",
                    "clin", "immu", "clin", "immu", "fname"),
...
Note that the coloured points indicate the results from a new simulations that I initiated after identifying a minor, but not ignorable, bug in the code.
...
df_tmp <- df_all %>%
  ...
  dplyr::filter(delta > 0 & !(accrual %in% c(30, 50)) & info_delay != 0.8 & basesero < 0.8) %>%
  dplyr::filter(date <= max_file_date) 
...
ggplot(df_tmp, aes(x = delta, y = prob_sup, 
  ...
  geom_line() + 
  geom_point(data = df_tmp2, aes(x = delta, y = prob_sup, 
                                 group = paste(accrual, ":", info_delay),
                                 colour = paste0(info_delay, " mnths"))) + 
  scale_color_manual(name = "Information delay", values = cbp[2:5]) +
...
Additional plots for other subsets of the parameter space (information delay is 1 month, accrual 40 per quarter).
...
df_tmp <- df_all %>%
  dplyr::mutate(delta = trtclin - baseclin,
    gamma = trtsero - basesero) %>%
  dplyr::filter(basesero %in% c(0.6,0.7,0.8) & gamma > 0) %>%
  dplyr::filter(date <= max_file_date) 


ggplot(df_tmp, aes(x = delta, y = prob_sup, 
  group = basesero, 
  linetype = paste(basesero, "/", trtsero))) +
  geom_line() + 
  scale_linetype_discrete(name = "Base/trt sero: ") +
  scale_x_continuous("Difference median survival time (mnths)") +
  scale_y_continuous("P(e.s)", breaks = seq(from = 0, to = 1, by = 0.1), lim = c(0, 1)) 
```
...
ggplot(df_tmp, aes(x = delta, y = prob_fut, 
                   ...
                   linetype = paste0(info_delay, " mnths"))) +
  geom_line() + 
  geom_point(data = df_tmp2, aes(x = delta, y = prob_fut, 
                   group = paste(accrual, ":", info_delay), 
                   colour = paste0(info_delay, " mnths"))) + 
  scale_color_manual(name = "Information delay", values = cbp[2:5]) +
  scale_linetype_discrete(name = "Information delay") +
...
df_tmp <- df_all %>%
  dplyr::mutate(delta = trtclin - baseclin,
    gamma = trtsero - basesero) %>%
  dplyr::filter(delta > 0 & !(accrual %in% c(30, 50)) & 
                  info_delay != 0.8 & delta == 5 & basesero < 0.8) %>%
  dplyr::filter(date <= max_file_date) 
...
ggplot(df_tmp, aes(x = gamma, y = prob_stopv, 
  ...
  linetype = paste0(info_delay, " mnths"))) +
  geom_line() + 
  geom_point(data = df_tmp2, aes(x = gamma, y = prob_stopv, 
                                 group = paste(accrual, ":", info_delay), 
                                 colour = paste0(info_delay, " mnths"))) + 
  scale_color_manual(name = "Information delay", values = cbp[2:5]) +
  scale_linetype_discrete(name = "Information delay") +
...
ggplot(df_tmp, aes(x = gamma, y = prob_stopv, 
                   group = basesero, 
                   colour = paste(basesero, "/", trtsero))) +
  geom_point() + 
  #scale_linetype_discrete(name = "Base/trt sero: ") +
  scale_color_discrete(name = "Base/trt sero: ") +
  scale_x_continuous("Difference in seroconversion rate") +
   scale_y_continuous("P(s.v.)", breaks = seq(from = 0, to = 1, by = 0.1), lim = c(0, 1))
```
...
ggplot(df_tmp, aes(x = delta, y = prob_incon, 
                   ...
                   linetype = paste0(info_delay, " mnths"))) +
  geom_line() + 
  geom_point(data = df_tmp2, aes(x = delta, y = prob_incon, 
                   group = paste(accrual, ":", info_delay), 
                   colour = paste0(info_delay, " mnths"))) + 
  scale_color_manual(name = "Information delay", values = cbp[2:5]) + 
  scale_linetype_discrete(name = "Information delay") +
  scale_x_continuous("Difference median survival time (mnths)") +
  scale_y_continuous("P(n.d)", breaks = seq(from = 0, to = 0.05, by = 0.01)) +
facet_grid(paste0("Accrual = ", accrual, " per qtr") ~ paste0("Diff in sero rate = ", gamma)) 
```
...
df_tmp <- df_all %>%
  dplyr::mutate(delta = trtclin - baseclin,
    gamma = trtsero - basesero) %>%
  dplyr::filter(delta > 0 & !(accrual %in% c(30, 50)) & info_delay == 1 & basesero < 0.8) %>%
  dplyr::filter(date <= max_file_date) 

df_tmp2 <- df_all %>%
  dplyr::mutate(delta = trtclin - baseclin,
    gamma = trtsero - basesero) %>%
  dplyr::filter(delta > 0 & !(accrual %in% c(30, 50)) & info_delay == 0 & basesero < 0.8) %>%
  dplyr::filter(date <= max_file_date) 
...
ggplot(df_tmp, aes(x = delta, y = ss_clin_mean)) +
  ...
  geom_point(data = df_tmp3, aes(x = delta, y = ss_clin_mean), colour = cbp[7]) + 
  scale_linetype_discrete(name = "Information delay") +
  scale_x_continuous("Difference median survival time (mnths)") +
...
ggplot(df_tmp, aes(x = delta, y = ss_clin_mean)) +
  geom_line(aes(group = basesero, linetype = paste(basesero, "/", trtsero))) + 
  geom_point() + 
  geom_errorbar(aes(ymin = ss_clin_mean - ss_clin_sd, 
    ymax = ss_clin_mean + ss_clin_sd), size = 0.2, width = 0.5) + 
  scale_linetype_discrete(name = "Base/trt sero: ") +
...
df_tmp <- df_all %>%
  dplyr::mutate(delta = trtclin - baseclin,
                gamma = trtsero - basesero) %>%
  dplyr::filter(delta > 0 & !(accrual %in% c(30, 50)) & 
                  info_delay == 1 & delta == 5 & basesero < 0.8) %>%
  dplyr::filter(date <= max_file_date) 
...
ggplot(df_tmp, aes(x = gamma, y = ss_immu_mean, 
  ...
  geom_line() + 
  geom_point(data = df_tmp2, aes(x = gamma, y = ss_immu_mean, 
                   group = accrual, 
                   colour = factor(accrual))) + 
...
ggplot(df_tmp, aes(x = gamma, y = ss_immu_mean))  +
  geom_jitter(width = 0.003, height = 0.003) + 
  scale_color_discrete(name = "Base/trt sero: ") +
  scale_x_continuous("Difference seroconversion rate", lim = c(0.05, 0.2)) +
...
The black dots show the results when the baseline seroconversion rate is 0.8, the treatment seroconversion rate is 0.9, the baseline time to event is 30 months and the treatment arm time to event ranges from 35 to 50. 
...
ggplot(df_tmp, aes(x = delta, y = p_c_intrm_vs_fin, 
  ...
  linetype = paste0(accrual)))  +
  geom_line() + 
  geom_point(data = df_tmp2, aes(x = delta, 
                                 y = p_c_intrm_vs_fin, 
                                 group = paste0(accrual))) +
  geom_point(data = df_tmp3, aes(x = delta, 
                                 y = p_c_intrm_vs_fin, 
                                 group = paste0(accrual),
                                 colour = paste0(accrual))) +
  scale_color_manual(name = "Accrual (n per qtr)", values = cbp[2:5]) + 
  scale_linetype_discrete(name = "Accrual (n per qtr)") +
  scale_x_continuous("Difference median survival time (mnths)") +
...
ggplot(df_tmp, aes(x = gamma, y = p_i_intrm_vs_fin, 
                   ...
                   linetype = paste0(accrual)))  +
  geom_line() + 
  geom_point(data = df_tmp2, aes(x = gamma, y = p_i_intrm_vs_fin, 
                                 colour = paste0(accrual))) + 
  scale_color_manual(name = "Accrual (n per qtr)", values = cbp[2:5]) + 
  scale_linetype_discrete(name = "Accrual (n per qtr)") +
  scale_x_continuous("Difference seroconversion rate") +
...
Table 6 shows the combined set of results generated in these simulations post 2019-03-13. These simulations were generated to see the effect of a minor bug fix in the code and constitute the latest most relevant results.
...
df_2a %>% 
  ...
  dplyr::mutate(delta = trtclin - baseclin,
                gamma = trtsero - basesero) %>%
  dplyr::select(-idsim, -trtclin, -trtsero, 
                -nsim, -alt_cen, -date, -fname) %>%
  dplyr::select(baseclin, delta, basesero, gamma, everything()) %>%
  DT::datatable(rownames = T, 
                class = 'cell-border stripe',
                caption="Table 6. Combined results",
                colnames = c("#","tte0", "delta_c",
                             "p0", "delta_i", 
                             "accrual", "info_delay",
                             "P(es)", "P(ef)", "P(sv)", "P(nd)",
                             "SS_clin",
                             "SS_sero",
                             "P(SS<250)", "P(SS<500)", "P(SS<750)", 
                             "Evnts",
                             "Win@Fin_clin", "Win@Fin_sero", 
                             "!FF_clin", "!FF_sero"),
                #filter = list(position = 'top', clear = TRUE, plain= FALSE),
                options=list(pageLength = nrow(df_2a),
                             #searchCols = list(rep(NULL, 21)),
                             autoWidth = T,
                             #columnDefs = list(list(width = '500px', targets = 0:18)),
                             dom = 't',
                             initComplete = JS(
                               "function(settings, json) {",
                               "$(this.api().table().header()).css({'font-size': '70%'});",
                               "}"))) %>%
  DT::formatStyle(columns = 0:20, fontSize = '70%')
```
...
kable(df_0 %>% 
  ...
  dplyr::select(-idsim, -nsim, -alt_cen, -date) %>%
    dplyr::arrange(desc(fname))  ,
      caption = paste0("Table 6. Combined Simulation Results (runs post ", max_file_date, ")"),
      col.names = c("ctl", "trt", "ctl", "trt", "accrual", "delay",
                    "e.s.", "e.f", "s.v.", "n.d", 
                    "enrolled", "blood res", "P(<250)", "P(<500)", "P(<750)", "evnts",
                    "clin", "immu", "clin", "immu", "fname"),
...
Table 7 shows the combined set of results generated in these simulations pre 2019-03-13. 
...
kable(df_0 %>% 
  ...
  dplyr::select(-idsim, -nsim, -alt_cen, -date) %>%
    dplyr::arrange(desc(fname))  ,
      caption = "Table 6. Combined Simulation Results",
      col.names = c("ctl", "trt", "ctl", "trt", "accrual", "delay",
                    "e.s.", "e.f", "s.v.", "n.d", 
                    "enrolled", "blood res", "P(<250)", "P(<500)", "P(<750)", "evnts",
                    "clin", "immu", "clin", "immu", "fname"),
