...
df_1 <- df_1 %>%
  dplyr::filter(!(date %in% as.POSIXct(c("2019-03-16 17:01:30", 
                                         "2019-03-16 01:53:36"))) )

df_1b <- df_all %>%
  dplyr::filter(date %in% no_fut_set)

stopifnot(nrow(df_1) == 6)
...
# post_final_thresh:
# used in the final analysis to assess both sero and tte endpoint posteriors
# 
# post_sero_win_thresh_start  
# we linearly interpolate between post_sero_win_thresh_start and 
...
# currently set to 0.95 for all interims up to nmaxsero then NA. used to check 
#                             
# whether a trial has 'won' during the futility assessment
# 
# post_tte_win_thresh_start   
# we linearly interpolate between post_tte_win_thresh_start and post_tte_win_thresh_end 
#                             
# to come up with a post_tte_win_thresh threshold at each interim. currently both set to 0.95 
# post_tte_win_thresh_end     
# post_tte_win_thresh         
# currently set to 0.95 for all interims. used to check whether a trial 
# has 'won' during the futility assessment
# 
# pp_sero_sup_thresh          
# currently set to 0.95 for all interims. used to assess whether 
# seroconversion ep should be stopped for superiority
# 
# post_tte_sup_thresh_start   
# we linearly interpolate between post_tte_sup_thresh_start and post_tte_sup_thresh_end 
#                             
# to come up with a post_tte_sup_thresh threshold at each interim. currently both set to 0.96
# post_tte_sup_thresh_end
# post_tte_sup_thresh         
# currently set to 0.96 for all interims. used to assess whether seroconversion ep 
#                             
# should be stopped for expected superiority
# 
# pp_sero_fut_thresh          
# futility check will stop the trial if the proportion of simulated trials from the 
#                            
...
d_thres <- data.frame(threshname = c("post_final_thresh",
                                     ...
                                     "pp_sero_fut_thresh",
                                     "pp_tte_fut_thresh"), 
                      thresh = c(0.95, 0.95, 0.96, 0.96, 0.95, 0.05, 0.05),
...
Contrastingly, Table 5 contains the results when the futility rule is remove on both endpoints. The results suggest that without the futility rule, the type-i error would not be controlled at the 0.05 level. Also, the trials will run for much longer, as evident from the large expected SS.
...
#   dplyr::select(basesero, trtsero, accrual, info_delay, gamma, prob_stopv)
# 
...
df_all %>% 
  ...
  DT::datatable(rownames = T, 
                ...
                options=list(pageLength = nrow(df_all),
