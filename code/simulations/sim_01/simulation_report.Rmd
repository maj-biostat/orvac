...
![Figure 1](images/typei_tpyeii.png "Figure 1")
...
for(fname in resfiles){
  ...
  # d1 <- readRDS(file.path(getwd(), "out", "res-2019-01-17-11-59-12.RDS"))
  ...
  df_res$trial_outcome <- factor(df_res$trial_outcome, levels = 1:3, labels = c("Futile", "Superior", "Inconclusive"))
  ...
  pct_fut <- 100*nrow(df_tmp)/max(df_res$idxsim)
  ...
  pct_sup <- 100*nrow(df_tmp)/max(df_res$idxsim)
  ...
  df_tmp <- df_res %>%
    dplyr::filter(stop_v_samp == 1)
  pct_stopv <-100*nrow(df_tmp)/max(df_res$idxsim)
